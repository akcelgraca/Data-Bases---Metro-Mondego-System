...
# 4. Testing Race Conditions (Vulnerability Demonstration)

A critical vulnerability exists inside vehicle seating logic within **Endpoint 11 (`/dbproj/ticket/use/`)**.

### Reproducing the Vehicle Overbooking Risk
...
3. Validate payload:
   ...
   ```
4. Query the journey capacity via `GET /dbproj/lines_next`. You will notice the `capacidade_disponivel` has decreased drastically and drops deep into negative logic (e.g., `-5`).

### Vulnerability Explanation
In `data_api.py`, Endpoint 11 acquires an exclusive row lock on the journey vehicle record object:
```sql
SELECT v.id FROM viagem v ... LIMIT 1 FOR UPDATE
```
However, **it never verifies if the remaining capacity is actually greater than zero!**
```sql
UPDATE viagem SET capacidade_disponivel = capacidade_disponivel - 1 WHERE id = %s
```
Because neither the Python application asserts `if capacidade_disponivel > 0:` after retrieving the lock, nor does the database schema possess a logical hard limit (`ALTER TABLE viagem ADD CHECK (capacidade_disponivel >= 0)`), identical concurrent validations simply bypass mathematical bottom limits, pushing vehicle availability arbitrarily down into negatives resulting in drastic spatial overbooking constraints.
